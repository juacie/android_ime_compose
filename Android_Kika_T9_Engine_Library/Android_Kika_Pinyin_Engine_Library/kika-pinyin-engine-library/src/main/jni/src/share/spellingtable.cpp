...
const char SpellingTable::
    kNotSupportList[kNotSupportNum][kMaxSpellingSize + 2] = {"HM", "HNG", "NG"};
...
const char* SpellingTable::arrange(size_t *item_size, size_t *spl_num) {
  ...
  if (need_score_) {
    ...
    for (size_t pos = 0; pos < spelling_num_; pos++) {
      ...
      if (need_score_) {
        if (0 == pos) {
          ...
          if (raw_spellings_[pos].freq == 0.0) {
            continue;
          }
    ...
    // 255 is reserved for virtual spellings
    score_amplifier_ = 1.0 * 253 / min_score;
...
unsigned char SpellingTable::get_average_score() {
  return average_score_;
    ...
    unsigned char SpellingTable::get_score_by_idx (uint16 idx) {
        return (unsigned char)spelling_buf_[idx * spelling_size_ + spelling_size_ - 1];
    }
