...
inline LmaScoreType UserDict::translate_score(int raw_score) {
  ...
  //uint64 lmt_off = extract_score_lmt(raw_score);
  ...
  *need_extend = false;
  ...
  while ((size_t)middle < max_off && !fuzzy_break && !prefix_break) {
    ...
    if (equal_spell_id(splids, nchar, &searchable) == true) {
      ...
      lpi_items[lpi_current].lmt = lmts_[middle];
      lpi_current++;
    }
    middle++;
  }
...
uint32 UserDict::_get_lemma_lmt(LemmaIdType lemma_id) {
    if (is_valid_state() == false)
        return 0;
    if (is_valid_lemma_id(lemma_id) == false)
        return 0;
...
uint32 UserDict::_get_lemma_lmt(char16 lemma_str[], uint16 splids[],
                               uint16 lemma_len) {
    if (is_valid_state() == false)
        return 0;
...
bool UserDict::reset(const char *file) {
  ...
  size_t wred = fwrite(&version, 1, sizeof(uint32), fp);
  UserDictInfo info;
...
bool UserDict::load(const char *file, LemmaIdType start_id) {
  ...
  lmts = (uint32 *)malloc((dict_info.lemma_count + kUserDictPreAlloc) << 2);
  if (!lmts) goto error;
  ...
  readed = 0;
  while (readed < toread && !ferror(fp) && !feof(fp)) {
    readed += fread((((uint8*)lmts) + readed), 1, toread - readed, fp);
  }
  if (readed < toread)
    goto error;
...
#ifdef ___SYNC_ENABLED___
void UserDict::write_back_sync(FILE* fp) {
  int err = fseek(fp, 4 + dict_info_.lemma_size
                  + (dict_info_.lemma_count << 3)
                  + (dict_info_.lemma_count << 2) // lmts
#ifdef ___PREDICT_ENABLED___
  ...
  if (err)
    return;
  fwrite(syncs_, 4, dict_info_.sync_count, fp);
  fwrite(&dict_info_, sizeof(dict_info_), 1, fp);
}
#endif

void UserDict::write_back_offset(FILE* fp) {
  int err = fseek(fp, 4 + dict_info_.lemma_size, SEEK_SET);
  if (err)
    return;
  fwrite(offsets_, 4, dict_info_.lemma_count, fp);
#ifdef ___PREDICT_ENABLED___
  fwrite(predicts_, 4, dict_info_.lemma_count, fp);
#endif
  fwrite(scores_, 4, dict_info_.lemma_count, fp);
  fwrite(lmts_, 4, dict_info_.lemma_count, fp);
#ifdef ___SYNC_ENABLED___
  fwrite(syncs_, 4, dict_info_.sync_count, fp);
#endif
  fwrite(&dict_info_, sizeof(dict_info_), 1, fp);
...
void UserDict::write_back_score(FILE* fp) {
  int err = fseek(fp, 4 + dict_info_.lemma_size
                  + (dict_info_.lemma_count << 2)
#ifdef ___PREDICT_ENABLED___
                  + (dict_info_.lemma_count << 2)
...
void UserDict::write_back_lemma(FILE* fp) {
  int err = fseek(fp, 4, SEEK_SET);
  if (err)
    return;
  ...
  err = fseek(fp, dict_info_.lemma_size - need_write, SEEK_CUR);
  if (err)
    return;
  fwrite(lemmas_ + dict_info_.lemma_size - need_write, 1, need_write, fp);
...
void UserDict::write_back_all(FILE* fp) {
  ...
  if (err)
    return;
  fwrite(lemmas_, 1, dict_info_.lemma_size, fp);
  fwrite(offsets_, 4, dict_info_.lemma_count, fp);
#ifdef ___PREDICT_ENABLED___
  fwrite(predicts_, 4, dict_info_.lemma_count, fp);
#endif
...
void UserDict::defragment(void) {
  ...
  while (first_freed < dict_info_.lemma_count) {
    ...
    tmp = lmts_[first_inuse];
    lmts_[first_inuse] = lmts_[first_freed];
    lmts_[first_freed] = tmp;
    // Swap ids_
...
void UserDict::reclaim() {
  ...
  for (int i = 0; i < rc; i++) {
    ...
    score_offset_pairs[i].lmt = lmts_[i];
  ...
  for (uint32 i = rc; i < dict_info_.lemma_count; i++) {
    ...
    if (s < score_offset_pairs[0].score) {
      ...
      score_offset_pairs[0].lmt = lmts_[i];
...
LemmaIdType UserDict::_put_lemma(char16 lemma_str[], uint16 splids[],
  ...
  int32 off = locate_in_offsets(lemma_str, splids, lemma_len);
  if (off != -1) {
    // from update_lemma(ids_[off], count, true);
    int delta_count = count;
    int score = scores_[off];
    int _count = extract_score_freq(score);
    if (_count + delta_count > kUserDictMaxFrequency ||
        _count + delta_count < _count) {
        delta_count = kUserDictMaxFrequency - _count;
    }
    _count += delta_count;
    dict_info_.total_nfreq += delta_count;
    scores_[off] = build_score(lmt, _count);
    lmts_[off] = (uint32)(lmt);
...
LemmaIdType UserDict::append_a_lemma(char16 lemma_str[], uint16 splids[],
  ...
  if (i != off) {
    ...
    temp = lmts_[off];
    memmove(lmts_ + i + 1, lmts_ + i, (off - i) << 2);
    lmts_[i] = temp;
