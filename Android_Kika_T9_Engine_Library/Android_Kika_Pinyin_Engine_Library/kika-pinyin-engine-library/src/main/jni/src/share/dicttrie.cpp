...
const unsigned char* DictTrie::load_dict(const unsigned char* mem) {
    if (NULL == mem)
        return NULL;

    lma_node_num_le0_ = *(uint32*)mem;
    mem += sizeof(uint32);
    lma_node_num_ge1_ = *(uint32*)mem;
    mem += sizeof(uint32);
    lma_idx_buf_len_ = *(uint32*)mem;
    mem += sizeof(uint32);
    top_lmas_num_ = *(uint32*)mem;
    mem += sizeof(uint32);
    ...
    memcpy(root_, mem, lma_node_num_le0_*sizeof(LmaNodeLE0));
    mem += lma_node_num_le0_*sizeof(LmaNodeLE0);
    memcpy(nodes_ge1_, mem, lma_node_num_ge1_*sizeof(LmaNodeGE1));
    mem += lma_node_num_ge1_*sizeof(LmaNodeGE1);
    memcpy(lma_idx_buf_, mem, lma_idx_buf_len_*sizeof(unsigned char));
    mem += lma_idx_buf_len_*sizeof(unsigned char);
...
bool DictTrie::load_dict(FILE *fp) {
  ...
  for (size_t i = 1; i < lma_node_num_le0_; i++) {
      for (uint16 splid = last_splid; splid < root_[i].spl_idx; splid++) {
          assert(splid >= kFullSplIdStart);
          splid_le0_index_[splid - kFullSplIdStart] = last_pos;
      }
      
      assert(root_[i].spl_idx >= kFullSplIdStart);
    splid_le0_index_[root_[i].spl_idx - kFullSplIdStart] =
        static_cast<uint16>(i);
    last_splid = root_[i].spl_idx;
    last_pos = i;
...
bool DictTrie::load_dict(const unsigned char *mem_sys_dict, LemmaIdType start_id,
                         LemmaIdType end_id) {
    if (NULL == mem_sys_dict || end_id <= start_id)
        return false;
    ...
    mem = spl_trie.load_spl_trie(mem_sys_dict);
    if (mem == NULL) {
        free_resource(true);
        return false;
    }
    
    mem = dict_list_->load_list(mem);
    if (mem == NULL) {
        free_resource(true);
        return false;
    }
  ...
  for (size_t son_pos = son_start; son_pos < son_end; son_pos++) {
    ...
    if (!cached && *lpi_num < lpi_max) {
      ...
      if (spl_trie_->is_half_id_yunmu(splid) && son_pos != son_start)
        ...
        if (need_lpi &&
            (!spl_homo_enabled || !spl_trie_->is_spl_homo_exception(splid, son->spl_idx)))
    ...
    for (uint16 ext_pos = 0; ext_pos < ext_num; ext_pos++) {
      ...
      for (size_t son_pos = 0; son_pos < (size_t)node->num_of_son; son_pos++) {
        ...
        if (son->spl_idx >= id_start
          ...
          if (*lpi_num < lpi_max
              && (!spl_homo_enabled
                  || !spl_trie_->is_spl_homo_exception(splid, son->spl_idx))) {
...
#define MAX_EXTENDBUF_LEN 200
  ...
  while (spl_pos < splid_str_len) {
    ...
    else if (spl_homo_enabled) {
        id_num = spl_trie_->get_spl_homo(splid_str[spl_pos]);
    }
      ...
      for (size_t node_fr_pos = 0; node_fr_pos < node_fr_num; node_fr_pos++) {
        ...
        for (size_t son_pos = son_start; son_pos < son_end; son_pos++) {
          ...
          if (node_to_num < MAX_EXTENDBUF_LEN) {
              if (!spl_homo_enabled
                  || !spl_trie_->is_spl_homo_exception(splid_str[spl_pos], node_son->spl_idx)) {
...
size_t DictTrie::predict_top_lmas(size_t his_len, NPredictItem *npre_items,
  ...
  while (item_num < npre_max && top_lmas_pos < top_lmas_num_) {
    ...
    npre_items[item_num].pre_len = (uint16)utf16_strlen(npre_items[item_num].pre_hzs);
    npre_items[item_num].his_len = (uint16)his_len;
