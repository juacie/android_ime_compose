...
bool MatrixSearch::init(const unsigned char *mem_sys_dict, const char *fn_usr_dict) {
    if (NULL == mem_sys_dict || NULL == fn_usr_dict)
        return false;
    
    if (!alloc_resource())
        return false;
    
    if (!dict_trie_->load_dict(mem_sys_dict, 1, kSysDictIdEnd))
        return false;
...
size_t MatrixSearch::search(const char *py, size_t py_len) {
  ...
  while (spl_id_num_ > kMaxLemmaSize + 1 || spl_id_num_ > kMaxSentenceLength) {
    py_len--;
...
size_t MatrixSearch::choose(size_t cand_id) {
  ...
  if (is_user_lemma(id_chosen)) {
    ...
    // patch: choose history to user dict
      if (NULL != user_dict_) {
          char16 word_str[kMaxLemmaSize + 1];
          uint16 spl_ids[kMaxLemmaSize];
          
          uint16 str_len = get_lemma_str(id_chosen, word_str, kMaxLemmaSize + 1);
          uint16 tmp = get_lemma_splids(id_chosen, spl_ids, cand_len, false);
          if (tmp == str_len) {
              user_dict_->put_lemma(word_str, spl_ids, str_len, 1);
          }
  ...
  // TODO: check this assert failed if choose an user word
...
bool MatrixSearch::add_char_qwerty() {
  ...
  for (uint16 ext_len = kMaxPinyinSize + 1; ext_len > 0; ext_len--) {
    ...
    for (PoolPosType dmi_pos = matrix_[oldrow].dmi_pos;
      ...
      // Try to extend the search range by adjusting id_num
      if (spl_homo_enabled) {
        if (dep_->id_num == 1) {
          uint16 id_num = spl_trie_->get_spl_homo(spl_idx);
          if (id_num != 0)
            dep_->id_num = id_num;
        }
      }
...
size_t MatrixSearch::get_lpis(const uint16* splid_str, size_t splid_str_len,
  ...
  if (splid_str_len > 1) {
    ...
    for (size_t pos = 0; pos < lpsi_num; pos++) {
      ...
      if (pos > 0 && utf16_strcmp(lpsis[pos].str, lpsis[pos - 1].str) == 0) {
        if (remain_num > 0 &&
                (((lpsis[pos].lpi.psb < lpsis[pos - 1].lpi.psb) && !is_user_lemma(lpsis[pos - 1].lpi.id))
            || is_user_lemma(lpsis[pos].lpi.id))) {
    ...
    for (size_t pos = 0; pos < num; pos++) {
      if (pos > 0 && lma_buf[pos].hanzi == lma_buf[pos - 1].hanzi) {
        ...
        if (remain_num > 0 &&
            ((lma_buf[pos].psb < lma_buf[pos - 1].psb && !is_user_lemma(lma_buf[pos - 1].id))
             || is_user_lemma(lma_buf[pos].id))) {
  ...
  if (sort_by_psb) {
    ...
    myqsort(lma_buf, num, sizeof(LmaPsbItem), cmp_lpi_custom);
  }
  return num;
}
...
size_t MatrixSearch::inner_predict(const char16 *fixed_buf, uint16 fixed_len,
  ...
  if (kMixedPredictSort) {
    myqsort(npre_items_, res_total, sizeof(NPredictItem),
            cmp_npre_by_hislen_and_hanzi_sp);
  } else if (kPreferLongHistoryPredict) {
    myqsort(npre_items_, res_total, sizeof(NPredictItem),
...
size_t MatrixSearch::get_predicts(const char16 fixed_buf[],
  ...
  return inner_predict(fixed_buf, fixed_len, predict_buf, buf_len);
...
#ifdef IQQI_EXTEND
    size_t MatrixSearch::get_lemma_id(const char16 *str, size_t str_len) {
        if (dict_trie_ == NULL)
            return 0;
        
        return dict_trie_->get_lemma_id(str, str_len);
    }
    
    size_t MatrixSearch::get_user_lemma_id(const char16 *str, size_t str_len, const char* splstr) {
        if (user_dict_ == NULL)
            return 0;
        
        const uint16 length = strlen(splstr);
        if (length >= kMaxLemmaSize)
            return 0;
        
        uint16 splidx[kMaxLemmaSize];
        uint16 start_pos[kMaxLemmaSize+1];
        bool last_is_pre = true;
        
        if (spl_parser_->splstr_to_idxs(splstr, length, splidx, start_pos, length, last_is_pre) != str_len)
            return 0;
        
        return user_dict_->get_lemma_id((char16*)str, splidx, str_len);
    }


    
    char16* MatrixSearch::get_candidate0_ex(char16 *cand_str, size_t max_len,
                                         uint16 *retstr_len,
                                         bool only_unfixed, uint16* score, uint16 *lemma_id) {
        if (score != NULL)
            *score = 0xffff;
        if (lemma_id != NULL)
            *lemma_id = 0;
      ...
      while (mtrx_nd != NULL) {
        ...
        if (lemma_id != NULL && id_num - zero_count == 1)
            *lemma_id = _id;
    ...
    char16* MatrixSearch::get_candidate_ex(size_t cand_id, char16 *cand_str,
                                        size_t max_len, uint16* uni_psb, uint16* lemma_id) {
      if (uni_psb == NULL || lemma_id == NULL)
          return get_candidate(cand_id, cand_str, max_len);
      ...
      if (0 == cand_id) {
          ...
          char16* rc = get_candidate0_ex(cand_str, max_len, NULL, false, uni_psb, lemma_id);
          size_t len = utf16_strlen(cand_str);
          *uni_psb = (len == 0)? 0xffff: *uni_psb/len;
          return rc;
      } else {
        cand_id--;
      }
      ...
      if (s_len > 0 &&  max_len > s_len) {
        utf16_strncpy(cand_str, s, s_len);
        cand_str[s_len] = (char16)'\0';
        *uni_psb = lpi_items_[cand_id].psb / lma_len;
        *lemma_id = id;
        return cand_str;
      }
...
/*
uint16 _get_spl_lma_len(const char* splstr, size_t length) {
    
    uint16 splidx[kMaxLemmaSize] = {0};
    uint16 start_pos[kMaxLemmaSize+1] = {0};
    bool last_is_pre = true;
    
    return spl_parser_->splstr_to_idxs(splstr, length, splidx, start_pos, length, last_is_pre);
}
*/

size_t MatrixSearch::get_spl_id_num(const char* py) {
