...
const unsigned char * NGram::load_ngram(const unsigned char *mem) {
    if (NULL == mem)
        return NULL;
    ...
    memcpy(freq_codes_, mem, kCodeBookSize*sizeof(LmaScoreType));
    mem += kCodeBookSize*sizeof(LmaScoreType);
    memcpy(lma_freq_idx_, mem, idx_num_*sizeof(CODEBOOK_TYPE));
    mem += idx_num_*sizeof(CODEBOOK_TYPE);
