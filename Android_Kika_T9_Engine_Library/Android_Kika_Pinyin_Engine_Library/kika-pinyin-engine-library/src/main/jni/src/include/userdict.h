...
#define COARSE_UTC(year, month, day, hour, minute, second) \
  ...
  uint32 _get_lemma_lmt(char16 lemma_str[], uint16 splids[], uint16 lemma_len);

  int _get_lemma_score(LemmaIdType lemma_id);
  uint32 _get_lemma_lmt(LemmaIdType lemma_id);
  ...
  void write_back_sync(FILE* fp);
#endif
  void write_back_score(FILE* fp);
  void write_back_offset(FILE* fp);
  void write_back_lemma(FILE* fp);
  void write_back_all(FILE* fp);
  void write_back();
