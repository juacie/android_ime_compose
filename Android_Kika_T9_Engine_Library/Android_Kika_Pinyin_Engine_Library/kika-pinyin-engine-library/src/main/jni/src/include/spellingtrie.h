...
// Used to record different homo (incluing tones) range for different levels
struct SpellingHomo {
    int level;
    int spelling_idx;
    int num;
...
class SpellingTrie {
 private:
  ...
  char *ym_buf_;
  uint32 ym_size_;  // The size of longest Yunmu string, '\0'included.
  uint32 ym_num_;
  ...
  // Spelling homo array
  SpellingHomo* homo_buf_;
  size_t homo_buf_num_;

  uint16 *homo_spl_exception_ids_;
  uint32 homo_spl_exception_column_size_;
  uint32 homo_spl_exception_row_size_;
  ...
  // Spelling homo management function
  void add_homo_search_item(size_t level, int spelling_idx, int num);
  void try_add_homo_item(size_t level, const char char_for_node,
                         size_t item_start_next, size_t i, SpellingNode *node_current);

  uint16 get_spl_id(const char* splstr);
  void build_homo_exception_table();
#endif
 ...
 public:
  ...
  inline static bool is_valid_spl_char(char ch) {
    return (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '1' && ch <= '5');
  }
  ...
  // Get spelling homo range
  uint16 get_spl_homo(uint16 spl_idx) const;
    
  // To check the child spl id is not extendable from the parent, only in homo mode.
  bool is_spl_homo_exception(uint16 spl_idx, uint16 son_spl_idx) const;
  ...
  // Load from mem, return NULL if fail
  const unsigned char* load_spl_trie(const unsigned char *mem);
  ...
  uint16 get_ym_id(const char* ym_str);
