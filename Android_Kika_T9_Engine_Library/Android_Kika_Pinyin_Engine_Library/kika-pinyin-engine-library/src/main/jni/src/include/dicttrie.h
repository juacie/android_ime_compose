...
class DictTrie : AtomDictBase {
 private:
  ...
  const unsigned char* load_dict(const unsigned char* mem);
  ...
  static const int kMaxMileStone = 512;
  static const int kMaxParsingMark = 4096;
 ...
 public:
  ...
  // Load a binary dictionary from memory
  bool load_dict(const unsigned char *mem_sys_dict, LemmaIdType start_id,
                 LemmaIdType end_id);
