...
class MatrixSearch {
 private:
  ...
  static const bool kMixedPredictSort = true;
  ...
  static const size_t kMaxLmaPsbItems = 5800;
  ...
  static const size_t kMtrxNdPoolSize = 800;

  // The size of the DMI node pool.
  static const size_t kDmiPoolSize = 2400;
 ...
 public:
  ...
  bool init(const unsigned char *mem_sys_dict, const char *fn_usr_dict);
...
#ifdef IQQI_EXTEND
    size_t get_lemma_id(const char16 *str, size_t str_len);
    size_t get_user_lemma_id(const char16 *str, size_t str_len, const char* splstr);
    char16* get_candidate0_ex(char16 *cand_str, size_t max_len, uint16 *retstr_len,
                              bool only_unfixed, uint16* score, uint16 *lemma_id);
    char16* get_candidate_ex(size_t cand_id, char16 *cand_str, size_t max_len,
                             uint16* uni_psb, uint16* lemma_id);
    size_t get_spl_id_num(const char* py);
#endif
};
}
