...
DicTraverseSession::DicTraverseSession(bool usesLargeCache)
        : mPrevWordIdCount(0), mProximityInfo(nullptr), mDictionary(nullptr),
          mSuggestOptions(nullptr), mDicNodesCache(usesLargeCache), mDicNodesCacheG(usesLargeCache),
          mMultiBigramMap(), mInputSize(0), mMaxPointerCount(1), mMultiWordCostMultiplier(1.0f) {
    // NOTE: mProximityInfoStates is an array of instances.
    // No need to initialize it explicitly here.
}

void DicTraverseSession::getSuggestions(ProximityInfo *proximityInfo, const Dictionary *dictionary,
    ...
    init(dictionary, ngramContext, suggestOptions);
    const SuggestInterface &suggest = suggestOptions->isGesture()
            ? SuggestInterface::getGestureSuggestInstance()
            : SuggestInterface::getTypingSuggestInstance();
    suggest.getSuggestions(proximityInfo, this, xcoordinates,
...
void DicTraverseSession::init(const Dictionary *const dictionary,
    ...
    if(mUseRNN) {
        mRnnContext.init(getDictionaryStructurePolicy());
    }
}
...
void DicTraverseSession::resetCacheG(const int thresholdForNextActiveDicNodes, const int maxWords) {
    mDicNodesCacheG.reset(thresholdForNextActiveDicNodes /* nextActiveSize */,
                         maxWords /* terminalSize */);
    mMultiBigramMap.clear();
}
