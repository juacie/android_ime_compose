...
/* static */ float Weighting::getSpatialCost(const Weighting *const weighting,
    ...
    case CT_TERMINAL: {
        if (parentDicNode && parentDicNode->mLastSkip != -1) {
            float cost = weighting->getTerminalSpatialCost(traverseSession, dicNode);
            int trailingSkips = dicNode->getInputIndex(0) - parentDicNode->mLastSkip;
            cost += 3 * trailingSkips;
            return cost;
        }
        else
...
/* static */ float Weighting::getLanguageCost(const Weighting *const weighting,
    ...
    case CT_COMPLETION:
        return 0.0f;
    case CT_TERMINAL: {
        float improbability = 0;
        if(traverseSession->useRNN()) {
            improbability = RNNUtils::getRNNNodeImprobability(traverseSession, dicNode, multiBigramMap);
        } else {
            improbability = DicNodeUtils::getBigramNodeImprobability(
                                    traverseSession->getDictionaryStructurePolicy(), dicNode, multiBigramMap);
        }

        return weighting->getTerminalLanguageCost(traverseSession, dicNode, improbability);
    }
    case CT_TERMINAL_INSERTION:
        return 0.0f;
    case CT_NEW_WORD_SPACE_SUBSTITUTION:
