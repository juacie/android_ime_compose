...
void SuggestionResults::outputSuggestions(int *outputCodePointsArray,
        int *outScoresArray, int *outSpaceIndicesArray,
        int *outTypesArray, int *outAutoCommitFirstWordConfidence) {
    int outputIndex = 0;
    while (!mSuggestedWords.empty()) {
        ...
        CharUtils::filterCodePoints(outputCodePointsArray + start,
                MAX_WORD_LENGTH /* maxLength */, suggestedWord.getCodePoint(),
                suggestedWord.getCodePointCount(), true /* needsNullTermination */);
        outScoresArray[outputIndex] = suggestedWord.getScore();
        outSpaceIndicesArray[outputIndex] = suggestedWord.getIndexToPartialCommit();
        outTypesArray[outputIndex] = suggestedWord.getType();

        if (mSuggestedWords.size() == 1) {
            *outAutoCommitFirstWordConfidence = suggestedWord.getAutoCommitFirstWordConfidence();
        }
