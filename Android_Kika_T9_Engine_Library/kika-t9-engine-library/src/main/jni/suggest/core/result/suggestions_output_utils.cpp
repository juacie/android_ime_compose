...
const int SuggestionsOutputUtils::MIN_LEN_FOR_MULTI_WORD_AUTOCORRECT = 16;
...
#else
        const int terminalSize = traverseSession->getDicTraverseCacheG()->terminalSize();
#endif
        std::vector<DicNode> terminals(terminalSize);
        for (int index = terminalSize - 1; index >= 0; --index) {
            traverseSession->getDicTraverseCacheG()->popTerminal(&terminals[index]);
        }
    ...
    /* static */ void SuggestionsOutputUtils::outputSuggestionsOfDicNodeGesture(
        ...
        const float compoundDistance = terminalDicNode->getNormalizedCompoundDistance();
...
/* static */ void SuggestionsOutputUtils::outputSuggestions(
    ...
    if(traverseSession->useRNN()) {
        RNNUtils::outputSuggestionsOfRNN(scoringPolicy, traverseSession, weightOfLangModelVsSpatialModelToOutputSuggestions, outSuggestionResults);
    }
...
/* static */ void SuggestionsOutputUtils::outputSuggestionsOfDicNode(
    ...
    if(DEBUG_RNN && true){
        int codePoints[MAX_WORD_LENGTH];
        terminalDicNode->outputResult(codePoints);
        char charPoint[MAX_WORD_LENGTH];
        int i = 0;
        for(i = 0; i < terminalDicNode->getTotalNodeCodePointCount(); i++) {
            charPoint[i] = codePoints[i];
        }
        charPoint[i] = 0;
        RNNLOGI("ngram %s spatial: %f, language: %f, compound: %f", charPoint,
               terminalDicNode->getSpatialDistanceForScoring(),
               terminalDicNode->getLanguageDictance(),
                compoundDistance);
    }
