...
class DicNodesCacheG {
 public:
    AK_FORCE_INLINE explicit DicNodesCacheG(const bool usesLargeCapacityCache)
    : mUsesLargeCapacityCache(usesLargeCapacityCache),
    mDicNodePriorityQueue0(getCacheCapacity()),
    mDicNodePriorityQueue1(getCacheCapacity()),
    mDicNodePriorityQueue2(getCacheCapacity()),
    ...
    AK_FORCE_INLINE virtual ~DicNodesCacheG() {}
 ...
 private:
    DISALLOW_COPY_AND_ASSIGN(DicNodesCacheG);
    ...
    AK_FORCE_INLINE static DicNodePriorityQueueG *moveNodesAndReturnReusableEmptyQueue(
                                                                                      DicNodePriorityQueueG *src, DicNodePriorityQueueG **dest) {
        const int srcMaxSize = src->getMaxSize();
        const int destMaxSize = (*dest)->getMaxSize();
        DicNodePriorityQueueG *tmp = *dest;
        *dest = src;
        (*dest)->setMaxSize(destMaxSize);
        tmp->clearAndResize(srcMaxSize);
        return tmp;
    ...
    const bool mUsesLargeCapacityCache;
    // Instances
    DicNodePriorityQueueG mDicNodePriorityQueue0;
    DicNodePriorityQueueG mDicNodePriorityQueue1;
    DicNodePriorityQueueG mDicNodePriorityQueue2;
    DicNodePriorityQueueG mDicNodePriorityQueueForTerminal;
    
    // Active dicNodes currently being expanded.
    DicNodePriorityQueueG *mActiveDicNodes;
    // Next dicNodes to be expanded.
    DicNodePriorityQueueG *mNextActiveDicNodes;
    // Cached dicNodes used for continuous suggestion.
    DicNodePriorityQueueG *mCachedDicNodesForContinuousSuggestion;
    // Current top terminal dicNodes.
    DicNodePriorityQueueG *mTerminalDicNodes;
    int mInputIndex;
    int mLastCachedInputIndex;
};
