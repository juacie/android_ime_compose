...
void Suggest::getSuggestions(ProximityInfo *pInfo, void *traverseSession,
    ...
    if(isGesture) {
        initializeSearchG(tSession);
        PROF_TIMER_END(0);
        
        PROF_TIMER_START(1);

        // keep expanding search dicNodes until all have terminated.
        while (tSession->getDicTraverseCacheG()->activeSize() > 0) {
            expandCurrentDicNodesGesture(tSession);
            tSession->getDicTraverseCacheG()->advanceActiveDicNodes();
            tSession->getDicTraverseCacheG()->advanceInputIndex(inputSize);
        }
        PROF_TIMER_END(1);
        PROF_TIMER_START(2);
...
void Suggest::initializeSearchG(DicTraverseSession *traverseSession) const {
    if (!traverseSession->getProximityInfoState(0)->isUsed()) {
        return;
    }
    
    if (traverseSession->getInputSize() > MIN_CONTINUOUS_SUGGESTION_INPUT_SIZE
        && traverseSession->isContinuousSuggestionPossibleG()) {
        // Continue suggestion
        traverseSession->getDicTraverseCacheG()->continueSearch();
    } else {
        // Restart recognition at the root.
        traverseSession->resetCacheG(TRAVERSAL->getMaxCacheSize(traverseSession->getInputSize(),
                                                               traverseSession->getSuggestOptions()->weightForLocale()),
                                    TRAVERSAL->getTerminalCacheSize());
        // Create a new dic node here
        DicNode rootNode;
        DicNodeUtils::initAsRoot(traverseSession->getDictionaryStructurePolicy(),
                                 traverseSession->getPrevWordIds(), &rootNode);
        traverseSession->getDicTraverseCacheG()->copyPushActive(&rootNode);
    }
}
        ...
        if (dicNode.isInDigraph()) {
            ...
            for (int i = 0; i < childDicNodesSize; ++i) {
                ...
                if (TRAVERSAL->isOmission(traverseSession, &dicNode, childDicNode,
                    ...
                    // T9: only operate when user input longer words
                    // TODO: consider merge into master. This is a general UX feedback from T9 team,
                    // who indicates insertion/omission can be frustrated to users when input size
                    // is small.
                    if (traverseSession->getInputSize() > 4) {
                        correctionDicNode.initByCopy(childDicNode);
...
void Suggest::createNextWordDicNode(DicTraverseSession *traverseSession, DicNode *dicNode,
    ...
    void Suggest::expandCurrentDicNodesGesture(DicTraverseSession *traverseSession) const {
        ...
        while (traverseSession->getDicTraverseCacheG()->activeSize() > 0) {
            DicNode dicNode;
            traverseSession->getDicTraverseCacheG()->popActive(&dicNode);
            ...
            if (point0Index > 0 && TRAVERSAL->isSkip(traverseSession, &dicNode)) {
                correctionDicNode.initSkip(&dicNode);
                processDicNodeAsSkipGesture(traverseSession, &correctionDicNode);
                correctionDicNode.clearSkip();
            }
    ...
    void Suggest::processExpandedDicNodeGesture(
        ...
        if (dicNode->getNormalizedCompoundDistance() < static_cast<float>(MAX_VALUE_FOR_WEIGHTING)) {
            if (dicNode->hasChildren()) {
                traverseSession->getDicTraverseCacheG()->copyPushNextActive(dicNode);
            }
        }
    }
    ...
    void Suggest::processTerminalDicNodeGesture(
        ...
        // Create a non-cached node here.
        DicNode terminalDicNode(*dicNode);
        Weighting::addCostAndForwardInputIndexGesture(WEIGHTING, CT_TERMINAL, traverseSession, dicNode,
                                               &terminalDicNode, traverseSession->getMultiBigramMap());
        traverseSession->getDicTraverseCacheG()->copyPushTerminal(&terminalDicNode);
    }
