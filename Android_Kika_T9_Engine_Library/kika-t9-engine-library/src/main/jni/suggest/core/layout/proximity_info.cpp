...
ProximityInfo::ProximityInfo(const int keyboardWidth, const int keyboardHeight,
    ...
    memcpy(mProximityCharsArray, proximityChars, sizeof(int) * GRID_WIDTH * GRID_HEIGHT * MAX_PROXIMITY_CHARS_SIZE);

    memcpy(mKeyXCoordinates, keyXCoordinates, sizeof(int) * KEY_COUNT);
    memcpy(mKeyYCoordinates, keyYCoordinates, sizeof(int) * KEY_COUNT);
    memcpy(mKeyWidths, keyWidths, sizeof(int) * KEY_COUNT);
    memcpy(mKeyHeights, keyHeights, sizeof(int) * KEY_COUNT);
    memcpy(mKeyCodePoints, keyCharCodes, sizeof(int) * KEY_COUNT);

    if (hasTouchPositionCorrectionData()) {
        memcpy(mSweetSpotCenterXs, sweetSpotCenterXs, sizeof(float) * KEY_COUNT);
        memcpy(mSweetSpotCenterYs, sweetSpotCenterYs, sizeof(float) * KEY_COUNT);
        memcpy(mSweetSpotRadii, sweetSpotRadii, sizeof(float) * KEY_COUNT);
    }
