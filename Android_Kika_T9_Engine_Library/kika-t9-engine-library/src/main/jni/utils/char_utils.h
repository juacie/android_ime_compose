...
class CharUtils {
 public:
    ...
    static unsigned filterCodePoints(int *buf, unsigned bufSize,
                                     const int *codePoints, unsigned n,
                                     bool needsNullTermination) {
        if (bufSize < (needsNullTermination ? n + 1 : n))
